 ...
 * @author kayra
 */
public class Vezir extends OyunNesnesi {
    Kaplamalar kaplama = Main.getir();
    
    public boolean renk = false; // false = beyaz, true = siyah
    
    public Vezir(int x, int y, ID id, boolean renk) {
        super(x, y, id);
        this.renk = renk;
    }
    ...
    public void ciz(Graphics2D g) {
        if(renk)g.drawImage(kaplama.taslar[0][5], this.x, this.y, null); //siyah
        else g.drawImage(kaplama.taslar[1][5], this.x, this.y, null); //beyaz
    }
