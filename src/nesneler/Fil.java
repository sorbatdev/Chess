 ...
 * @author kayra
 */
public class Fil extends OyunNesnesi {
    Kaplamalar kaplama = Main.getir();
    
    public boolean renk = false; // false = beyaz, true = siyah
    
    public Fil(int x, int y, ID id, boolean renk) {
        super(x, y, id);
        this.renk = renk;
    }
    ...
    public void ciz(Graphics2D g) {
        if(renk)g.drawImage(kaplama.taslar[0][3], this.x, this.y, null); //siyah
        else g.drawImage(kaplama.taslar[1][3], this.x, this.y, null); //beyaz
    }
